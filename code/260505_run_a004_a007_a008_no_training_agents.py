...
Output:
  data_out/rl_sims/c{ts}_a004_a007_a008_no_training_agents.csv
"""
...
from yoked_rl_runner import GraphMazeEnv


TARGET_ANIMALS = ['004', '007', '008', '044']
INCLUDE_a033_POST_260314 = False
...
def main():
    ...
    out = PROJECT_ROOT / 'data_out' / 'rl_sims' / f'{ts}_a004_a007_a008_no_training_agents.csv'
    ...
    for sess in sessions:
        ...
        def _make_env():
            return GraphMazeEnv(
                ...
                node_xy_coordinate_list=st_pos,
                start_node=start_node,
                rwd_per_node_initial=rewards.copy(),
        ...
        for seed in range(N_REPEATS):
            # Heuristics — gym-env-based agents need a fresh env per call
            for agent_name, fn in [
                ('Random',         lambda s=seed: run_random_agent(_make_env(), s)),
                ('Random_forward', lambda s=seed: run_forward_random_agent(_make_env(), s)),
                ('Greedy_oracle',  lambda s=seed: run_greedy_agent(_make_env(), s)),
                ('FullFwd',        lambda s=seed: FullFwdAgent().run_episode(seed=s, **common)),
                ('NoBkFullFwd',    lambda s=seed: NoBkFullFwdAgent().run_episode(seed=s, **common)),
                ('FSC_bio',        lambda s=seed: FSCBioAgent().run_episode(seed=s, **common)),
            ]:
                try:
                    rew = fn()
